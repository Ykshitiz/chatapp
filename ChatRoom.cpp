...
void Room::deliver(ParticipantPointer participant,Message&message){
    ...
    while(!messageQueue.empty()){
        ...
        for(ParticipantPointer _particitant:participants){
            if(participant!= _particitant ){
                _particitant->write(msg);
            }
        }
    }
}
...
void Session::write(Message &message){
    ...
    while(messageQueue.size()!=0){
        ...
        if(header_decode){
            std::string dody = message.getBody();
            async_write(dody ,message.getBodyLength());
        }else{
...
void accept_connection(boost::asio::io_context&io,char *port,tcp::acceptor &acceptor,Room &room
